...
class UpdateRegionCodes extends ConsoleCommand
{
    ...
    protected function configure()
    {
        ...
        $this->addOption('db-ip-csv', null, InputOption::VALUE_OPTIONAL, 'Uses the provided DB IP CSV database to iterate over all included IP ranges.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($dbIpCsvFile)) {
            $this->enrichWithDbIpRegions($dbIpCsvFile, $newRegions, $output);
        }
...
CONTENT;
    ...
    private function enrichWithDbIpRegions(string $dbIpCsvFile, array &$regions, OutputInterface $output)
    {
        $output->writeln('Start looking through GeoIP database for nmissing region names');
        ...
        $progress = new ProgressBar($output, 29406553);
